...
public:
    ...
    void addMidiData(const MidiMessage& m) {
    ...
    void noteOn(int note, int velocity) {
        _keyboard.notes[note].velocity = velocity;
        _keyboard.notes[note].sustained = false;
    }

    void noteOff(int note) {
        if (_sustainPedalOn) {
            _keyboard.notes[note].sustained = true;
        } else {
            _keyboard.notes[note].velocity = 0;
            _keyboard.notes[note].sustained = false;
        }
    }
    ...
    void sustainOff() {
        ...
        for (auto& [velocity, sustained] : _keyboard.notes) {
            if (sustained) {
                velocity = 0;
                sustained = false;
            }
    ...
    [[nodiscard]] common::Keyboard getState() const {
        return _keyboard;
    }

private:
    common::Keyboard _keyboard;
...
public:
        ...
        _dataThread = std::thread([this]() {
            ...
            while (_isRunning.load() == true) {
                ...
                if (message.size() == 3) {
                    ...
                    auto state = _midiHandler.read();
                    state.addMidiData(MidiMessage{status, note, velocity});
                    _midiHandler.write(state);
                }
    ...
    [[nodiscard]] common::Keyboard getKeyboardState() const {
        return _midiHandler.read().getState();
    }
...
private:
    ...
    common::AtomicSnapshot<MidiHandler> _midiHandler;
};
...
common::Keyboard MidiInputStream::getKeyboardState() const {
    return _impl->getKeyboardState();
}
